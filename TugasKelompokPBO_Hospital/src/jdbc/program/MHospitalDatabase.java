...
import jdbc.model.DataManager;
import jdbc.service.DAOManager;
import java.time.LocalDate;

public class MHospitalDatabase {
    public static void main(String[] args) {
        try {
            ...
            System.out.println("Dimasukkan kembali : Dokter ke-2 = " + koleksiDokter.getIsi(2).getName());
            ...
            // ==========================================
            // ALIRAN PROSES PERSISTENCE KE DATABASE MYSQL
            // ==========================================
            System.out.println("=== MEMULAI PROSES SIMPAN DATA KE DATABASE ===");
            DAOManager manager = new DAOManager();
            
            manager.getHospitalDAO().saveHospital(hospital);
            manager.getHospitalDAO().saveDoctor(dokter1);
            manager.getHospitalDAO().saveDoctor(dokter2);
            manager.getHospitalDAO().saveDoctor(dokter3);
            manager.getHospitalDAO().saveNurse(perawat1);
            manager.getHospitalDAO().saveNurse(perawat2);
            manager.getHospitalDAO().savePatient(pasien1);
            manager.getHospitalDAO().savePatient(pasien2);
            manager.getHospitalDAO().savePatient(pasien3);
            ...
            // Tambahan untuk menampung objek Kamar (Room) Kelompok 6
            ...
            manager.getHospitalDAO().saveBill(tagihan1);
            manager.getHospitalDAO().saveBill(tagihan2);
            manager.getHospitalDAO().saveBill(tagihan3);
            
            // <-- Kodingan barumu untuk menyimpan data Rekam Medis (Medical Record) Pasien:
            manager.getHospitalDAO().saveMedicalRecord(pasien1);
            manager.getHospitalDAO().saveMedicalRecord(pasien2);
            manager.getHospitalDAO().saveMedicalRecord(pasien3);
            
            System.out.println("\n[SUKSES RELASIONAL] Seluruh objek simulasi Kelompok 6 berhasil masuk ke database pbo!");
